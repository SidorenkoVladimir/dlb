...
/*
 * Персональный логотип и ссылка прописат ьв function.php
 */
function sid_edit_admin_logo(){?>
    <style type="text/css">
        body.login div h1 a{
            background: transparent url("<?php echo get_template_directory_uri(); ?>/img/logo.png") 50% 50% no-repeat!important;
            width: 212px!important;
            height: 51px!important;
        }
    </style>
<?php }
add_action('login_enqueue_scripts', 'sid_edit_admin_logo');

function sid_edit_admin_logo_link(){
    return home_url('/');
}
add_filter('login_headerurl', 'sid_edit_admin_logo_link');

function sid_change_toolbar(){
    global $wp_admin_bar;
...
add_action('admin_bar_menu', 'sid_change_toolbar', 40);

function sid_remove_foto_link(){
    echo '<span id="footer-thankyou">Copyrights <a href="http://dlb.loc/">DLB.LOC</a>.</span>';
}
add_filter('admin_footer_text', 'sid_remove_foto_link');